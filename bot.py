...
rarityAr = ['common bass','common pike','common grunt','common\'t cod','common\'t marlin','common\'t tang','rare snapper','rare tetra','rare firefish']
...
async def prepare(ctx, slot:int):
    if (slot>len(users.get(f'{ctx.author.id}').get('inv'))):
        ...
        await ctx.channel.send(f'{ctx.author}, you were able to increase the value of this fish by {skill}! It is now worth {await value(r,q,f,skill)} perles! :cook:')
        with open('users.json', 'w') as outfile:
            json.dump(users, outfile)
    else:
...
async def cook(ctx, slot:int):
    if (slot <= len(users[f'{ctx.author.id}']['inv'])):
        ...
        if yum < 3:
            users[f'{ctx.author.id}']['money'] -= 3-yum
