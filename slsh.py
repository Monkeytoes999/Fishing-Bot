...
class BotClient(discord.Client):
    ...
    async def setup_hook(self):
        ...
        try:
            ...
            await self.tree.sync(guild=discord.Object(id=1047327559140003921))
...
async def on_interaction(ctx):
    ...
    if (ctx.extras.get("cook") != None):
        file = open('users.json')
        u = json.load(file)
        users[f'{ctx.user.id}'] = u[f'{ctx.user.id}']
...
async def fish(ctx: discord.Interaction):
    ...
    if (userInfo["isFishing"] == 0 and (userInfo["lastFish"] - time.time() < 0)):
        ...
        numCaught = numCaught + round(random.triangular(0, users[f'{ctx.user.id}']["equipment"]['fishEq']["ef"], users[f'{ctx.user.id}']["equipment"]['fishEq']["efc"]))
        r = 0
        q = 0
...
async def cook(ctx: discord.Interaction, slot:int=1):
    u = await cookF.cook(ctx, users, slot)
    users[f'{ctx.user.id}'] = u[f'{ctx.user.id}']
    with open('users.json', 'w') as outfile:
        json.dump(users, outfile)
...
async def sell(ctx: discord.Interaction, slot:int=1):
    u = await cookF.sell(ctx, users, slot)
    users[f'{ctx.user.id}'] = u[f'{ctx.user.id}']
    with open('users.json', 'w') as outfile:
        json.dump(users, outfile)
